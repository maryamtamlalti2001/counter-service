...
ARG PYTHON_VERSION=3.13.0a6
#change the version to alpine to avoid problems 
FROM python:${PYTHON_VERSION}-alpine3.18 AS base

# Update system and critical libraries
RUN apk update && apk add --no-cache libexpat expat-dev && \
    apk upgrade

# Prevents Python from writing pyc files.
ENV PYTHONDONTWRITEBYTECODE=1

# Keeps Python from buffering stdout and stderr to avoid situastions where
